...
void BSTCorrection(BTNode *root)
{
    ...
    BTNode **first = (BTNode **)malloc(sizeof(BTNode *));
    BTNode **middle = (BTNode **)malloc(sizeof(BTNode *));
    BTNode **last = (BTNode **)malloc(sizeof(BTNode *));
    *first = *middle = *last = NULL;

    while (!isEmptyQueue(q) && q.size > 1)
    {
        ...
        for (int i = 0; i < q.size; i++)
        {
            ...
            {
                if (*first == NULL) // first wrong node not yet found
                {
                    *first = nodeB;
                    *middle = nodeA;
                }
                else // first wrong node has alr been found
                {
                    *last = nodeB;
                }
            }
    ...
    int temp;
    if (*first != NULL && *last != NULL) // two wrong nodes
    {
        temp = (*first)->item;
        (*first)->item = (*last)->item;
        (*last)->item = temp;
    }
    else // only one wrong node, swap wrong node and its adjacent node
    {
        temp = (*first)->item;
        (*first)->item = (*middle)->item;
        (*middle)->item = temp;
    }
}