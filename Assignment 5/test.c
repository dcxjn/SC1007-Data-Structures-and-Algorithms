...
int HashDelete(HashTable *Q3HashPtr, int key)
{
    ...
    ListNode *head, *temp;
    head = temp = Q3HashPtr->Table[index].head;

    Q3HashPtr->Table[index].size -= 1;
    Q3HashPtr->nSize -= 1;

    if (head->key == HashSearch(*Q3HashPtr, key)->key)
    {
        Q3HashPtr->Table[index].head = head->next;
        return 1;
    }

    while (temp->next != NULL)
    {
        if (temp->next->key == HashSearch(*Q3HashPtr, key)->key)
        {
